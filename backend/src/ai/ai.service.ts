...
export class AiService {
    ...
    private buildPrompt(documentText: string, documentTitle: string): string {
        return `Проанализируй входящий документ транспортной компании. Определи тип, категорию, сделай сводку и предложи отдел для маршрутизации. Все ответы должны быть на русском языке, даже если документ на другом языке.
